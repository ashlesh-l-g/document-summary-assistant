...
describe('Groq Provider', () => {
  ...
  it('triggers timeout when request exceeds configured timeout duration', async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(AIProviderError);
      const providerErr = err as AIProviderError;
      expect(providerErr.statusCode).toBe(408);
      expect(providerErr.isRetryable).toBe(true);
      expect(providerErr.message).toContain('timed out');
    }
  });

  it('respects caller-provided AbortSignal and aborts immediately', async () => {
